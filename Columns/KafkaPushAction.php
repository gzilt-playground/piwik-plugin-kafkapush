 ...
 *
 */
namespace Piwik\Plugins\KafkaPush\Columns;
...
class KafkaPushAction extends ActionDimension
{
    ...
    {
        return Piwik::translate('KafkaPush_KafkaPushAction');
    }

    protected function generateKafkaToken($seed = '') {
    ...
    protected function configureSegments()
    {
        ...
        $segment->setName('KafkaPush_KafkaPushAction');
    ...
    {
        $settings = new \Piwik\Plugins\KafkaPush\Settings();

        $rec = array('uid' => $visitor->getVisitorColumn('user_id'),
                     'sid' => $request->getIdSite(),
                     'action' => $action->getActionType(),
                     'uuid' => $this->generateKafkaToken());

        $rk = new \RdKafka\Producer();
        $rk->setLogLevel(LOG_DEBUG);
        $rk->addBrokers($settings->brokers->getValue());
        $topic = $rk->newTopic($settings->topic->getValue());

        $topic->produce(RD_KAFKA_PARTITION_UA, 0, json_encode($rec));
        if($settings->debugMode->getValue()) {
            Common::printDebug('KafkaPush'.json_encode($rec));
        }
